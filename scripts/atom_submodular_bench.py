"""D04 + D06: atom retrieval + submodular set-cover selection + Qwen 7B."""
...
def main():
    ...
    ap.add_argument("--adaptive_top_k", action="store_true",
                    help="Use top_k*1.5 if intent==ANY, top_k otherwise")
    ...
    for qi, q in enumerate(queries):
        ...
        # Adaptive top_k: ANY-intent queries (often multi-faceted) get wider net
    ...
    for qi, (q, sel) in enumerate(zip(queries, selected_per_q, strict=True)):
        ...
        prompt = assemble(query=q["text"], evidence=evidence, style=args.prompt_style)
        ...
        out_rows.append({
            "query_id": q["query_id"],
            "answer_text_full": gen,
            "answer_text": ans_for_metric,
